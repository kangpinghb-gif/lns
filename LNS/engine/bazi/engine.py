...
def _calculate_start_age(year_stem: str, gender: str,
    ...
    try:
        terms = _get_jie_datetimes(birth_date.year)
        previous_terms = [dt for dt in terms if dt <= birth_dt]
        next_terms = [dt for dt in terms if dt >= birth_dt]

        if forward and next_terms:
            delta_days = (next_terms[0] - birth_dt).total_seconds() / 86400
        elif not forward and previous_terms:
            delta_days = (birth_dt - previous_terms[-1]).total_seconds() / 86400
        else:
            delta_days = 15.0

        start_age = delta_days / 3.0
        return round(max(0.0, min(start_age, 10.0)), 1)
    except Exception:
        return 5.0


def _get_jie_datetimes(year: int) -> List[datetime]:
    """获取指定年前后可用的节气时间，用于近似起运年龄。"""
    terms = []
    for y in (year - 1, year, year + 1):
        terms.extend(_get_year_jie_datetimes(y))
    return sorted(terms)


def _get_year_jie_datetimes(year: int) -> List[datetime]:
    ...
    for name, value in table.items():
        if name not in MONTH_BRANCH_BY_TERM:
            continue
...
def _coerce_datetime(value) -> Optional[datetime]:
    ...
    if isinstance(value, str):
        try:
            ...
            return dt_parse(value).replace(tzinfo=None)
        except Exception:
            return None
    if all(hasattr(value, attr) for attr in ("getYear", "getMonth", "getDay")):
        hour = value.getHour() if hasattr(value, "getHour") else 0
        minute = value.getMinute() if hasattr(value, "getMinute") else 0
        second = value.getSecond() if hasattr(value, "getSecond") else 0
        return datetime(value.getYear(), value.getMonth(), value.getDay(), hour, minute, second)
    if hasattr(value, "toYmdHms"):
        return _coerce_datetime(value.toYmdHms())
