...
class StateEngine:
    ...
    def process(self, bazi_output: BaZiOutput, age: float = 25.0) -> StateOutput:
        ...
        # 4. 当前大运主题
        result.luck_cycle_theme = self._calc_luck_cycle_theme(bazi_output, age)
    ...
    def _get_current_luck_cycle(self, bazi: BaZiOutput, age: float) -> Optional[object]:
        """按当前年龄匹配大运周期。"""
        if not bazi.luck_cycles:
            return None

        for cycle in bazi.luck_cycles:
            parts = str(cycle.age_range or "").replace("~", "-").split("-")
            if len(parts) != 2:
                continue
            try:
                start_age = float(parts[0])
                end_age = float(parts[1])
            except ValueError:
                continue
        ...
        if bazi.start_age >= 0:
            cycle_index = max(0, math.floor((age - bazi.start_age) / 10))
            if cycle_index < len(bazi.luck_cycles):
                return bazi.luck_cycles[cycle_index]
    ...
    def _calc_luck_cycle_theme(self, bazi: BaZiOutput, age: float) -> List[str]:
        """从当前大运干支推导语义主题"""
        themes = []
        cycle = self._get_current_luck_cycle(bazi, age)
        if not cycle:
            return themes
