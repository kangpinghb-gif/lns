(function () {
  // 自动检测子路径前缀（如 /lns/），开发模式（file:）直接连 8080
  var _BASE_PATH = (function () {
    var p = window.location.pathname;
    var m = p.match(/^(\/[^/]+)\//);
    return m ? m[1] : "";
  })();
  var API_BASE = window.__LNS_API_BASE || (window.location.protocol === "file:" ? "http://127.0.0.1:8080" : _BASE_PATH);
